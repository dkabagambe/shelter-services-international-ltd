...
function FooterLink({ label }: { label: string }) {
  ...
  if (isInternal) {
    ...
    return (
      <Link to={href.split("?")[0] as "/"} search={href.includes("?") ? Object.fromEntries(new URLSearchParams(href.split("?")[1])) as any : undefined} className={cls}>
        {label}
      </Link>
    );
  }
  ...
  return (
    <footer className="bg-[#111c17] text-gray-300">
      <div className="shell grid gap-12 py-16 lg:grid-cols-[1.6fr_repeat(3,1fr)]">
        ...
        <div>
          <Link to="/" className="flex items-center gap-3">
            ...
            <div className="leading-tight">
              ...
              <p className="text-[10px] font-semibold tracking-[0.18em] text-[#1a6b3c] uppercase">International Limited</p>
            </div>
          </Link>
          ...
          <ul className="mt-6 space-y-3">
            <li className="flex items-start gap-3 text-sm">
              ...
              <span className="text-gray-400">Enterprise Road, Industrial Area, Nairobi, Kenya</span>
            </li>
            <li className="flex items-center gap-3 text-sm">
              <Phone className="size-4 shrink-0 text-[#1a6b3c]" />
              <a href="tel:+254703372539" className="text-gray-300 transition-colors hover:text-white">
                +254 703 372 539
              </a>
            </li>
            <li className="flex items-center gap-3 text-sm">
              <Mail className="size-4 shrink-0 text-[#1a6b3c]" />
              <a href="mailto:info@shelterservices.co.ke" className="text-gray-300 transition-colors hover:text-white">
        ...
        {columns.map((col) => (
          <nav key={col.title}>
            <h3 className="mb-5 text-sm font-bold uppercase tracking-wider text-white">{col.title}</h3>
