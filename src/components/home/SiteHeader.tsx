...
import {
  ...
  LogOut, LayoutDashboard, Leaf, Apple, Beef, Carrot, LayoutGrid,
} from "lucide-react";
...
export function SiteHeader() {
  ...
  function handleSearch(e: React.FormEvent) {
    e.preventDefault();
    if (search.trim()) navigate({ to: "/shop", search: { q: search.trim() } as any });
  }
    ...
    <header className="sticky top-0 z-50 border-b border-border bg-white shadow-sm">
      ...
      <div className="shell flex items-center gap-4 py-3 lg:gap-6">
        ...
        <div className="ml-auto flex shrink-0 items-center gap-3 lg:ml-0">
              ...
              <>
                ...
                >
                  ...
                  <span className="leading-tight text-left">
                    <span className="block text-xs text-gray-500">{isAdmin ? "Admin" : "My Account"}</span>
                ...
                {userMenuOpen && (
                  <div className="absolute right-0 top-full mt-2 w-48 rounded-xl border border-gray-200 bg-white py-1 shadow-lg z-50">
                    ...
                    <button
                      onClick={async () => { setUserMenuOpen(false); await signOut(); navigate({ to: "/" }); }}
          ...
          {/* Cart */}
          <button onClick={openCart} aria-label="Open cart" className="relative flex items-center gap-2.5">
      ...
      <div className="hidden border-t border-gray-200 bg-white lg:block">
        <div className="shell flex items-center gap-0 py-0">
          ...
          <div className="flex shrink-0 items-center gap-2 py-2">
            <Link to="/checkout">
              <Button variant="outline" size="sm" className="border-[#1a6b3c] text-[#1a6b3c] hover:bg-[#1a6b3c] hover:text-white">
                Request a Quote
              </Button>
            </Link>
            <Link to="/shop">
        ...
        <div className="border-t border-gray-200 bg-white lg:hidden">
          ...
          <form onSubmit={handleSearch} className="shell pt-4">
            <div className="flex h-10 items-center overflow-hidden rounded-lg border border-gray-300 bg-white focus-within:border-[#1a6b3c]">
              ...
              />
              <button type="submit" className="flex h-full items-center bg-[#1a6b3c] px-3 text-white">
                <Search className="size-4" />
              </button>
            </div>
          </form>
          ...
          <nav className="shell flex flex-col py-4">
            ...
            <div className="mt-3 mb-1">
              <p className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2">Browse by Category</p>
            ...
            {user ? (
              ...
              <div className="mt-3 flex gap-2 border-t border-gray-100 pt-3">
                <Link to="/login" onClick={() => setMobileOpen(false)} className="flex-1">
                  <Button variant="outline" className="w-full border-[#1a6b3c] text-[#1a6b3c]">Login</Button>
                </Link>
                <Link to="/signup" onClick={() => setMobileOpen(false)} className="flex-1">
                  <Button className="w-full bg-[#1a6b3c] text-white hover:bg-[#145530]">Sign Up</Button>
                </Link>
              </div>
            )}
            ...
            <div className="mt-3 flex gap-2 border-t border-gray-100 pt-3">
              <Link to="/checkout" onClick={() => setMobileOpen(false)} className="flex-1">
                <Button variant="outline" className="w-full border-[#1a6b3c] text-[#1a6b3c]">Request a Quote</Button>
              </Link>
              <Link to="/shop" onClick={() => setMobileOpen(false)} className="flex-1">
                <Button className="w-full bg-[#1a6b3c] text-white hover:bg-[#145530]">Order Now</Button>
              </Link>
            </div>
          </nav>
        </div>
